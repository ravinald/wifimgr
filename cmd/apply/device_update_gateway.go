...
func NewGatewayUpdater() *GatewayUpdater {
	return &GatewayUpdater{
		BaseDeviceUpdater: NewBaseDeviceUpdater("gateway"),
	}
}

// GetConfiguredDevices extracts Gateway MAC addresses from site configuration
func (g *GatewayUpdater) GetConfiguredDevices(siteConfig SiteConfig) []string {
	configuredGateways := make([]string, 0, len(siteConfig.Devices.WanEdge))

	// WanEdge is a map with MAC as key (JSON tag is "gateway")
	for mac := range siteConfig.Devices.WanEdge {
		normalizedMAC := macaddr.NormalizeOrEmpty(mac)
		if normalizedMAC != "" {
			configuredGateways = append(configuredGateways, normalizedMAC)
		}
	}
...
func (g *GatewayUpdater) FindDevicesInventoryStatus(client vendors.Client, cfg *config.Config, configuredGateways []string, siteName string) ([]DeviceInventoryStatus, error) {
	...
	for _, mac := range configuredGateways {
	...
	logging.Debugf("Inventory status check completed for %d gateways", len(statusList))
...
func (g *GatewayUpdater) FindDevicesToUpdate(ctx context.Context, client vendors.Client, _ *config.Config, siteConfig SiteConfig, configuredGateways []string, siteID string, apiLabel string) ([]string, error) {
	...
	g.batchLoader = batchLoader // Store for reuse - eliminates duplicate API call
	logging.Debugf("Batch loader created with %d devices for comparison", batchLoader.GetDeviceCount())

	// Get managed keys for gateway devices from the API-specific config
	managedKeys := getManagedKeysForDevice(apiLabel, "gateway")

	gatewaysToUpdate := make([]string, 0)

	for _, mac := range configuredGateways {
		// Get desired config from site configuration
		desiredConfig, found := g.GetDeviceConfigFromSite(siteConfig, mac)
		if !found {
			logging.Warnf("Gateway %s is in the list but not found in site configuration", mac)
			continue
		}
		...
		if err != nil {
			...
			fmt.Printf("-> gateway %s: not found in API cache for this site (skipping diff)\n", mac)
			continue
		}
		...
		logging.Debugf("Gateway %s - Current config: %+v", mac, currentConfig)
		logging.Debugf("Gateway %s - Desired config: %+v", mac, desiredConfig)
		logging.Debugf("Gateway %s - Needs update: %v", mac, needsUpdate)

		if needsUpdate {
			gatewaysToUpdate = append(gatewaysToUpdate, mac)
			logging.Debugf("Gateway %s needs configuration update", mac)
			...
			if viper.GetBool("show_diff") || viper.GetString("logging.level") == "debug" {
				...
				showGatewayConfigDiffWithManagedKeys(mac, currentConfig, desiredConfig, managedKeys, siteName)
			}
		} else {
			logging.Debugf("Gateway %s configuration is up to date", mac)
		}
	}

	return gatewaysToUpdate, nil
}

// UpdateDeviceConfigurations applies configuration updates to Gateways
func (g *GatewayUpdater) UpdateDeviceConfigurations(ctx context.Context, client vendors.Client, cfg *config.Config, siteConfig SiteConfig, macs []string, siteID string, apiLabel string) ([]string, error) {
	...
	for _, mac := range macs {
		...
		// Expand template references (device_template)
		expandedConfig, err := expandDeviceConfigWithTemplates(gatewayConfig, siteConfig)
		if err != nil {
			logging.Warnf("Error expanding templates for Gateway %s: %v - using unexpanded config", mac, err)
		} else {
			gatewayConfig = expandedConfig
		}
		...
		if len(managedKeys) > 0 {
			...
			logging.Debugf("Filtered config to %d managed keys for Gateway %s", len(filteredConfig), mac)
		} else {
...
// GetDeviceConfigFromSite extracts Gateway-specific config from site configuration
func (g *GatewayUpdater) GetDeviceConfigFromSite(siteConfig SiteConfig, mac string) (map[string]any, bool) {
	normalizedTargetMAC := macaddr.NormalizeOrEmpty(mac)

	// Try direct lookup first (in case keys match exactly)
	if gatewayConfig, ok := siteConfig.Devices.WanEdge[normalizedTargetMAC]; ok {
		return gatewayConfig, true
	}
	if gatewayConfig, ok := siteConfig.Devices.WanEdge[mac]; ok {
		return gatewayConfig, true
	}

	// Iterate and compare normalized versions (handles colon-separated keys)
	for configMac, gatewayConfig := range siteConfig.Devices.WanEdge {
...
// showGatewayConfigDiffWithManagedKeys displays a colored JSON diff with managed keys highlighted
func showGatewayConfigDiffWithManagedKeys(mac string, currentConfig, desiredConfig map[string]any, managedKeys []string, siteName string) {
	...
	// Debug logging
	logging.Debugf("Gateway %s - Filtered current fields: %v", mac, getMapKeys(filteredCurrent))
	logging.Debugf("Gateway %s - Filtered desired fields: %v", mac, getMapKeys(filteredDesired))
